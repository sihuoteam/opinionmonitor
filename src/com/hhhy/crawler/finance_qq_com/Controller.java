...
public class Controller {
    ...
    public void parseBoard(String keyWord,String BASE_URL) {
        ...
        String html = GetHTML.getHtml("http://www.sogou.com/sogou?site=finance.qq.com&query="+transKey+"&pid=sogou-wsse-b58ac8403eb9cf17-0017&sourceid=&idx=f", "UTF-8");
        ...
        Elements flag = document.select("div.wrap").select("div#wrapper").select("div#main").select("div.results").select("div[id].rb");
        if(flag.size()==0){
            //Todo ??
            System.out.println("nothing to show....");
        }
        else{
            Elements tableEles = document.select("div.wrap").select("div#wrapper").select("div#main").select("div.results").select("div[id].rb");
    ...
    public void parsePages(ArrayList<Element> tableList){
        for(Element ele:tableList){
            ...
            String time = FormatTime.getTime(ele.select("div.fb").select("cite").first().text(),"\\d{2}小时前");
            if(FormatTime.isAfterToday(time)){
                if(!this.spyHistory.contains(title)){
                    ...
                    System.out.println("website:"+"腾讯财经");
                    System.out.println("----------------");
                    this.spyHistory.add(title);
                    //调接口~~~~~
                }
            }
        }
